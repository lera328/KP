          ...
          <div className="card border-0 shadow-sm rounded-4 mb-3">
            <div className="card-body p-4 d-flex flex-wrap align-items-center gap-3">
              ...
              <div className="flex-grow-1" style={{ minWidth: 200 }}>
                <h1 className="fw-semibold mb-1" style={{ fontSize: '1.6rem' }}>
                  {fullName}
                </h1>
...
const AttendanceWidget = ({ items, periodDays, onPeriodChange, loading, onTileClick }) => {
  ...
  filtered.forEach((item, idx) => {
    if (idx === nowIndex) {
      tiles.push(<NowTile key="__now__" />);
    }
    tiles.push(
      <WidgetTile
        key={item.lesson_id}
      ...
      <div className="card-body p-3">
        ...
        {items.length === 0 ? (
          <div className="text-muted small text-center py-3">
            За выбранный период занятий не найдено.
          </div>
        ) : (
...
const WidgetTile = ({ item, todayStr, onClick }) => {
  ...
  return (
    <button
      ...
      style={{
        background: meta.bg,
        border: `1px solid ${isToday ? '#111827' : meta.border}`,
        borderRadius: 8,
        padding: '6px 8px',
        minWidth: 56,
        textAlign: 'center',
      ...
      <div className="modal-dialog modal-lg modal-dialog-centered">
        <div className="modal-content rounded-4 border-0 shadow-lg">
          ...
          <div className="modal-body pt-0">
            ...
            {item.description && (
              <InfoBlock title="Описание занятия" text={item.description} />
            )}
            {item.homework && (
              <InfoBlock title="Домашнее задание" text={item.homework} />
            )}
            {item.teacher_comment && (
              <InfoBlock
                title="Комментарий преподавателя"
                text={item.teacher_comment}
              />
            )}
            {!item.description && !item.homework && !item.teacher_comment && (
              <div className="text-muted small">
                {item.is_past
                  ? 'Преподаватель не оставил дополнительной информации.'
